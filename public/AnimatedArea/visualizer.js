...
class AlgorithmVisualizer {
    constructor(containerId, config) {
        ...
        // 初始化二叉树动画器（修改为动态导入）
        // 检查BinaryTreeAnimator是否可用
        if (typeof BinaryTreeAnimator !== 'undefined') {
            this.binaryTreeAnimator = new BinaryTreeAnimator(this.ctx, this.config);
        } else {
            console.warn('BinaryTreeAnimator未加载，二叉树功能将不可用');
        }
    ...
    drawBinaryTree() {
        ...
        // 检查BinaryTreeAnimator是否可用
        if (this.binaryTreeAnimator) {
            // 使用BinaryTreeAnimator绘制二叉树
            this.binaryTreeAnimator.draw(this.animationState);
        } else {
            this.drawNoDataMessage();
            console.warn('BinaryTreeAnimator未初始化，无法绘制二叉树');
        }
    }
    ...
    drawHighlightRanges(positionMap, layout, barWidth) {
        if (this.animationState.highlightRange && this.animationState.highlightRange.length > 0) {
            const ranges = this.animationState.highlightRange;
            const ctx = this.ctx;
            ...
            ranges.forEach((range, rangeIndex) => {
                if (range && range.length === 2) {
                    ...
                    if (start >= 0 && end >= 0 && positionMap.has(start) && positionMap.has(end)) {
                        // 为不同范围选择不同的高亮颜色
                        const colors = ['rgba(173, 216, 230, 0.3)', 'rgba(255, 182, 193, 0.3)', 'rgba(152, 251, 152, 0.3)'];
                        const colorIndex = rangeIndex % colors.length;
                        
                        // 绘制高亮背景
                        const startX = positionMap.get(start).originalX - spacing/2;
                        const endX = positionMap.get(end).originalX + barWidth + spacing/2;
                        const width = endX - startX;
                        const height = this.canvas.height - this.config.canvas.headerHeight;
                        const y = this.canvas.height - height;
                        
                        // 绘制矩形背景
                        this.drawRect(startX, y, width, height, {
                            fillStyle: colors[colorIndex],
                            strokeStyle: colors[colorIndex].replace('0.3', '0.6'), 
                            stroke: true
                        });
                        
                        // 可选：在区域上方添加标签
                        if (end - start > 2) { // 只为较大的区域添加标签
                            this.drawText(`区间 [${start}..${end}]`, startX + width/2, y - 5, {
                                fillStyle: '#555',
                                fontSize: '12px'
                            });
                        }
    ...
    calculateArrayLayout(n, maxValue) {
        ...
        return { barWidth, spacing, startX, maxHeight };
    }
    ...
    generatePreorderSteps(node, steps = [], path = []) {
        if (!this.binaryTreeAnimator) {
            console.warn('BinaryTreeAnimator未初始化，无法生成前序遍历步骤');
            return steps;
        }
        return this.binaryTreeAnimator.generatePreorderSteps(node, steps, path);
    }
    
    // 修改生成中序遍历步骤的方法
    generateInorderSteps(node, steps = [], path = []) {
        if (!this.binaryTreeAnimator) {
            console.warn('BinaryTreeAnimator未初始化，无法生成中序遍历步骤');
            return steps;
        }
        return this.binaryTreeAnimator.generateInorderSteps(node, steps, path);
    }
    
    // 修改生成后序遍历步骤的方法
    generatePostorderSteps(node, steps = [], path = []) {
        if (!this.binaryTreeAnimator) {
            console.warn('BinaryTreeAnimator未初始化，无法生成后序遍历步骤');
            return steps;
        }
        return this.binaryTreeAnimator.generatePostorderSteps(node, steps, path);
    }
    
    // 修改查找节点的方法
    findNodeById(root, id) {
        if (!this.binaryTreeAnimator) {
            console.warn('BinaryTreeAnimator未初始化，无法查找节点');
            return null;
        }
        return this.binaryTreeAnimator.findNodeById(root, id);
    }
    
    // 修改初始化二叉树的方法
    initializeBinaryTree() {
        if (!this.binaryTreeAnimator) {
            console.warn('BinaryTreeAnimator未初始化，无法初始化二叉树');
            return;
        }
        ...
        // 使用BinaryTreeAnimator的方法创建二叉树
        const tree = this.binaryTreeAnimator.createBinaryTree(initialData);
