...
try {
  ...
  socket.onopen = () => {
    ...
    // 如果Vue应用已就绪，确保更新消息处理程序
    if (window.app) window.app.checkWebSocketConnection();
  };
  ...
  socket.onclose = (event) => {
    ...
    // 如果Vue应用已就绪，尝试重新连接
    if (window.app && typeof window.app.attemptReconnect === 'function') {
      window.app.attemptReconnect(1);
    }
  };
  ...
  socket.onmessage = (event) => {
    try {
      ...
      // 如果Vue应用已就绪，使用它的处理方法
      if (window.app && typeof window.app.handleWebSocketMessage === 'function') {
        window.app.handleWebSocketMessage(response);
      } else {
        // 否则存储输出结果，供Vue实例稍后使用
        if (response.status === 'success') {
          window._pendingOutput = response.output;
        }
      }
    } catch (error) {
...
function handleError(error, options = {}) {
  ...
  const opts = {
    logToConsole: true,
    showToUser: true,
    type: 'general',    // 可能的类型: general, visualization, connection, debug
    attempt: 0,
  ...
  // 提取错误消息
  const errorMessage = opts.message || (error instanceof Error ? error.message : String(error));
  const logMessage = opts.logPrefix ? `${opts.logPrefix}: ${errorMessage}` : errorMessage;

  // 记录到控制台
  if (opts.logToConsole) {
    console.error(`[${opts.type}]: ${logMessage}`);
  }
  ...
  if (opts.showToUser) {
    if (opts.type === 'visualization') {
      ...
      // 自动切换到结果标签页显示错误
      if (opts.type === 'general' || opts.type === 'debug') {
        window.app.switchTab('result');
      }
    }
  }
  ...
  switch (opts.type) {
    ...
    case 'debug':
      // 特殊处理调试错误
      if (window.app && window.app.isDebugging) {
        window.app.debugMessages.push({ type: 'error', text: errorMessage });
        window.app._updateDebugConsole();
      }
      break;
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  new Vue({
    ...
    data: {
      ...
      problems: [], // 初始化为空数组，问题会通过Monaco验证器动态添加
      // 编辑器实例
      editorManager: null,
      isEditorReady: false,
      // 终端和Socket引用
    ...
    mounted() {
      ...
      window.addEventListener('editor-ready', () => {
        ...
        // 移除终端初始化逻辑，因为它已经在terminal-manager.js中处理了
        // 只保留全局终端对象引用
    ...
    methods: {
      ...
      // 切换输出标签页
      switchTab(tabName) {
        console.log(`开始切换到标签页: ${tabName}, 当前标签页: ${this.activeOutputTab}`);
        
        // 保存之前的标签页
        const previousTab = this.activeOutputTab;
      ...
      changeEditorLanguage(language) {
        if (this.editorManager && typeof this.editorManager.setLanguage === 'function') {
          this.editorManager.setLanguage(language);
        }
        // 确保无论如何都更新Vue实例的language值
        this.editorSettings.language = language;
      },

      // 更新编辑器主题并同时更新页面主题
      changeEditorTheme(theme) {
        console.log('切换编辑器主题:', theme);
        // 直接调用editorManager的方法
        if (this.editorManager && typeof this.editorManager.changeEditorTheme === 'function') {
          console.log('使用editorManager.changeEditorTheme方法');
          this.editorManager.changeEditorTheme(theme);
        } else {
          console.log('使用备用主题切换方法');
          // 备用处理以防editorManager不可用
          ...
          if (editorContainer) {
            // 移除编辑器容器上的现有主题类
            editorContainer.classList.remove('editor-theme-light', 'editor-theme-dark', 'editor-theme-high-contrast');
            
            // 添加新的主题类到编辑器容器
            if (theme === 'vs-dark') {
              editorContainer.classList.add('editor-theme-dark');
            } else if (theme === 'vs') {
              editorContainer.classList.add('editor-theme-light');
            } else if (theme === 'hc-black') {
              editorContainer.classList.add('editor-theme-high-contrast');
            }
          }
          
          // 将用户主题保存到本地存储
          localStorage.setItem('preferred-theme', theme);
          console.log('编辑器主题已保存到localStorage:', theme);
        }
      },

      // 运行代码
      runCode() {
        // 使用editorManager的runCode方法
        if (this.editorManager && typeof this.editorManager.runCode === 'function') {
          this.editorManager.runCode();
        } else {
          handleError('编辑器未初始化', { showToUser: true });
        }
      },
      ...
      initializeCanvas(attempt = 0) {
        ...
        if (!canvas) {
          if (attempt < 3) {
            ...
            // 稍后再尝试初始化
            setTimeout(() => this.initializeCanvas(attempt + 1), 500 * (attempt + 1));
          } else {
            handleError('找不到canvas元素，无法初始化可视化', {
              type: 'visualization',
              showToUser: true
            });
          }
      ...
      _doInitializeVisualizer() {
        Vue.nextTick(() => {
          try {
            ...
            console.error('_doInitializeVisualizer错误:', error);
            this.retryVisualizerInit(1);
          }
        });
      },
      ...
      resetAnimation() {
        ...
        if (this.visualizer) {
          ...
          if (this.selectedAlgorithm === 'linkedList') {
            ...
            this.visualizer.animationState.path = [];
            this.visualizer.animationState.description = '选择二叉树算法，点击开始按钮查看演示';
          } else if (this.selectedAlgorithm && ['bubbleSort', 'selectionSort', 'insertionSort', 'quickSort'].includes(this.selectedAlgorithm)) {
            this.visualizer.resetAnimationState(this.sortArray);
          } else {
      ...
      // 保留重置可视化方法作为别名，以保证向后兼容
      resetVisualization() {
        this.resetAnimation();
      },
      
      // 基础重置动画状态 - 只重置状态变量但不影响可视化器
      resetAnimationState() {
        this.hasStarted = false;
        this.isPlaying = false;
        this.isPaused = false;
      ...
      generateVisualizationData() {
        ...
        // 排序算法的处理逻辑
        // 解析代码中的数组
        let newArray = this.editorManager.extractArrayFromCode(code);
      ...
      stepForward() {
        ...
        if (this.visualizer && step) {
          // 聚合当前步骤信息用于日志
          const stepInfo = {
            index: this.currentAnimationStep,
            total: this.animationSteps.length,
          ...
          if (step.currentArray) {
            stepInfo.arrayElements = step.currentArray.length > 10 
              ? [...step.currentArray.slice(0, 10), '...'] 
              : step.currentArray;
          }
          
          // 记录步骤执行信息
          console.log(`步进执行: [${stepInfo.index+1}/${stepInfo.total}] ${stepInfo.description || stepInfo.type}`, stepInfo);
      ...
      handleKeydown(event) {
        // 如果事件来自编辑器，不处理热键
        const isInEditor = this.editorManager && typeof this.editorManager._isEventFromEditor === 'function' ? 
                           this.editorManager._isEventFromEditor(event) : false;
        if (isInEditor) {
          return;
        }
      ...
      stepBackward() {
        ...
        if (this.visualizer && step) {
          ...
          // 记录步骤执行信息
          console.log(`步进后退: [${stepInfo.index+1}/${stepInfo.total}] ${stepInfo.description || stepInfo.type}`, stepInfo);
      ...
      checkWebSocketConnection() {
        ...
        socket.onmessage = (event) => {
          try {
            const response = JSON.parse(event.data);
            // 将消息处理委托给handleWebSocketMessage方法
        ...
        // 处理可能存在的待处理输出
        if (window._pendingOutput) {
          this.writeToOutput(window._pendingOutput);
          this.switchTab('result');
          window._pendingOutput = null;
        }
      ...
      playNextStep() {
        // 聚合日志信息，而不是分散输出
        const stepInfo = {
          currentStep: this.currentAnimationStep + 1,
          totalSteps: this.animationSteps?.length || 0,
        ...
        if (!this.isPlaying || this.isPaused || !this.animationSteps.length) {
          console.log(`动画控制: 无法播放 - 状态[播放=${stepInfo.isPlaying}, 暂停=${stepInfo.isPaused}, 步骤=${stepInfo.currentStep}/${stepInfo.totalSteps}]`);
          return;
        }
        ...
        if (this.currentAnimationStep < this.animationSteps.length) {
          ...
          // 将步骤相关信息添加到聚合日志中
          stepInfo.stepType = step.type || 'unknown';
          stepInfo.description = step.description;
          
          // 添加数组元素信息（如果存在）
          if (step.currentArray) {
            stepInfo.arrayElements = step.currentArray.slice(0, 10); // 只显示前10个元素避免日志过长
            if (step.currentArray.length > 10) {
              stepInfo.arrayElements.push('...');
            }
          }
          
          // 添加其他重要信息
          if (step.comparing) stepInfo.comparing = step.comparing;
          if (step.swapping) stepInfo.swapping = step.swapping;
          if (step.sorted) stepInfo.sortedCount = step.sorted.length;
          if (step.countingArray) stepInfo.countingArray = step.countingArray.slice(0, 5);

          // 输出聚合的日志信息
          console.log(`动画步骤[${stepInfo.currentStep}/${stepInfo.totalSteps}]: ${stepInfo.description || stepInfo.stepType}`, stepInfo);
      ...
      handleVisualizationStep(data) {
        ...
        const stepInfo = {
          index: data.index,
          total: data.total,
          stepType: data.step?.type || 'unknown',
          description: data.step?.description
        };
        
        // 仅在关键节点输出日志，如首个和最后一个步骤
        if (data.index === 0 || data.index === data.total - 1 || data.index % 10 === 0) {
          console.log(`可视化进展: 步骤${data.index+1}/${data.total}`, stepInfo);
        }
      ...
      initLinkedListControls() {
        // 使用独立的链表动画器模块
        if (window.linkedListAnimator) {
          window.linkedListAnimator.initControls(this);
        } else {
          this.writeToOutput('错误：链表动画器未加载，请确保已引入LinkedListAnimator.js');
        }
      },
      ...
      handleLinkedListAction(action, value, position) {
        // 使用独立的链表动画器模块处理链表操作
        if (window.linkedListAnimator) {
          window.linkedListAnimator.handleAction(action, value, position);
        } else {
          this.writeToOutput('错误：链表动画器未加载，请确保已引入LinkedListAnimator.js');
        }
      },
      ...
      handleWebSocketMessage(response) {
        try {
          // 根据消息类型分发处理
          const type = response.type || response.status;
          ...
          switch(type) {
            ...
            case 'visualization_steps_ready':
              // 标记可视化已准备就绪
            ...
            case 'error':
              // 修改：优先使用output字段显示错误详情，而不仅仅是message
              const errorContent = response.output || response.message || '未知错误';
              // 直接输出到输出面板
              this.writeToOutput(`错误: ${errorContent}`);
              this.switchTab('result');
              
              // 还需要记录日志，但不一定要显示给用户
              if (errorContent !== response.message && response.message) {
                console.error(`[${response.message}]: ${errorContent}`);
              }
            ...
            default:
              ...
              if (response.status === 'error') {
                // 同样优先使用output字段
                const errMsg = response.output || response.message || '未知错误';
                this.writeToOutput(`错误: ${errMsg}`);
                this.switchTab('result');
                
                // 记录日志
                if (errMsg !== response.message && response.message) {
